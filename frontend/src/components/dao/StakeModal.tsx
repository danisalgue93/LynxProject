      ...
      <motion.div initial={{ opacity: 0, scale: 0.98, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: 12 }} className="relative w-full max-w-md bg-[#0D0D0E] border border-[#1F1F23] rounded-xl overflow-hidden shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input type="number" value={String(amount)} onChange={(e) => setAmount(Number(e.target.value))} className="w-full p-3 bg-[#141417] border border-[#1F1F23] rounded text-white outline-none" />
          </div>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button disabled={isSubmitting || !amount || amount <= 0} onClick={handleSubmit} className={cn('px-6 py-2 rounded font-black uppercase text-sm', isSubmitting ? 'bg-[#00FFD1]/30 text-black/50' : 'bg-[#00FFD1] text-black')}>{isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin mr-2 inline"/> {t('common.processing','Processing')}</> : t('governance.stakeNow','Stake')}</button>
          </div>
        </div>
      </motion.div>
    </div>
